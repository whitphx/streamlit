...
async def _mpa_v1(main_script_path: str) -> None:
...
class ScriptRunner:
    ...
    def start(self) -> None:
        ...
        # Stlite: wrap the coroutine so that each session has different cwd and home dir.
        from stlite_lib.server.task_context import DirectorySyncCoroutineProxy
        coro = DirectorySyncCoroutineProxy(self._run_script_thread())
        self._script_task = asyncio.create_task(coro)
